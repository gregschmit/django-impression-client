...
class RemoteImpressionServer(models.Model):
    ...
    the active server, or if none is active, then the
    ``IMPRESSION_CLIENT_DEFAULT_TARGET`` and ``IMPRESSION_CLIENT_DEFAULT_TOKEN`` Django
    settings values.
    """
    ...
    def get_target_and_token(cls):
        ...
        # return defaults
        return (
            get_setting("IMPRESSION_CLIENT_DEFAULT_TARGET"),
            get_setting("IMPRESSION_CLIENT_DEFAULT_TOKEN"),
        )
